...
def main():
    ...
    psilo_schaefer_before_dataset = BrainGraphDataset(img_dir=os.path.join(root, dataroot),
                                ...
                                transform=None, extra_data=data_labels, setting='graph_and_baseline')
    ...
    configs = [
        (psilo_ica_before_dataset, 'vgae_no_nf_ica_32_8.pt', 'ica'),
        (psilo_ica_before_dataset, 'vgae_no_nf_schaefer_32_8.pt', 'schaefer'),
        (psilo_aal_before_dataset, 'vgae_no_nf_aal_32_8.pt', 'aal'),
    ]
    ...
    for config in configs:
        ...
        for t, train_loader in enumerate(train_loaders):
            ...
            vgae = VGAE(1, 1, num_rois, 32, 8, device, dropout=0, l2_strength=0.001, use_nf=False).to(device)
            ...
            for epoch in tqdm(range(num_epochs)):
                ...
                # training
                model.train()
                for batch_idx, ((graph, baseline_bdi), label) in enumerate(train_loader):
                ...
                val_base = []
                with torch.no_grad():
                    for batch_idx, ((graph, baseline_bdi), label) in enumerate(val_loader):
                        graph = graph.to(device)  # move data to device
                        baseline_bdi = baseline_bdi.to(device)
                        label = label.to(device)
        ...
        filename = os.path.join(root, 'vgae_mlp_results', f'featureless-vgae-{config[2]}-{dropout}-results.csv')
